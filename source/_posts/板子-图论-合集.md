...
> 用途：树上基础操作（如深度、父节点、子树信息）模板
> 复杂度：预处理 O(n)，单次查询视功能而定（常见 O(1) / O(log n)）
...
> 用途：最大流/最小割建模（如二分图匹配、容量约束）
> 复杂度：Dinic 常见 O(m√n)~O(n^2m)，实际依图而定
