---
title: 板子：数论 合集
date: 2026-03-23 19:47:30
categories:
  - 算法板子
  - 数论
tags:
  - 板子
  - 数论
  - C++
---

> 本文整合了【数论】分类下的所有模板。

## FWT

> 用途：按位卷积（OR/AND/XOR 卷积）
> 复杂度：O(n log n)

```cpp
#include <bits/stdc++.h>
using namespace std;

using ll = long long;
const int M = 998244353;
const int N = 20;

void fmt_or(ll* a, int n, int type) {
    for (int k = 1;k < n;k <<= 1) {
        for (int i = 0;i < n;i += k << 1) {
            for (int j = 0;j < k;j++) {
                a[i + j + k] = (a[i + j + k] + a[i + j] * type + M) % M;
            }
        }
    }
}
void fmt_and(ll* a, int n, int type) {
    for (int k = 1;k < n;k <<= 1) {
        for (int i = 0;i < n;i += k << 1) {
            for (int j = 0;j < k;j++) {
                a[i + j] = (a[i + j] + a[i + j + k] * type + M) % M;
            }
        }
    }
}
void fwt_xor(ll* a, int n, int type) {
    for (int k = 1;k < n;k <<= 1) {
        for (int i = 0;i < n;i += k << 1) {
            for (int j = 0;j < k;j++) {
                ll x = a[i + j], y = a[i + j + k], t = type == 1 ? 1 : M + 1 >> 1;
                a[i + j] = (x + y) * t % M;
                a[i + j + k] = (x - y + M) * t % M;
            }
        }
    }
}
ll A[1 << N], B[1 << N], a[1 << N], b[1 << N];
template<typename F>
void calc(F f, int n) {
    for (int i = 0;i < n;i++) {
        a[i] = A[i];b[i] = B[i];
    }
    f(a, n, 1);
    f(b, n, 1);
    for (int i = 0;i < n;i++)a[i] = a[i] * b[i] % M;
    f(a, n, -1);
    for (int i = 0;i < n;i++)cout << a[i] << ' ';
    cout << '\n';
}
int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);
    int n;
    cin >> n;
    n = 1 << n;
    for (int i = 0;i < n;i++)cin >> A[i];
    for (int i = 0;i < n;i++)cin >> B[i];
    calc(fmt_or, n);
    calc(fmt_and, n);
    calc(fwt_xor, n);
    return 0;
}
```

## gcd,lcm,exgcd,ksm

> 用途：数论基础：gcd/lcm、扩展欧几里得、快速幂
> 复杂度：gcd/exgcd O(log n)，ksm O(log n)

```cpp
ll gcd(ll a, ll b) { return b ? gcd(b, a % b) : a; }
ll lcm(ll a, ll b) { return a * b / gcd(a, b); }

ll exgcd(ll a, ll b, ll& x, ll& y) {
    if (b == 0) {
        x = 1, y = 0;
        return a;
    }
    ll d = exgcd(b, a % b, x, y);
    ll t = x;
    x = y, y = t - a / b * y;
    return d;
}
ll d = exgcd(a, b, x, y);(d是gcd(a,b))
x = x * c / d, y = y * c / d;(求特解)
b /= d, a /= d;
x + b * k , y - a * k(k是整数,求通解)
求最小x技巧:
b = abs(b);
ll ans = (x % b + b) % b;
若求绝对值x最小:ans=min(ans,b-ans);

ll ksm(ll a, ll b) {
    ll res = 1;
    while (b) {
        if (b & 1)res = res * a % M;
        a = a * a % M;
        b >>= 1;
    }
    return res;
}

void bit(int x) {
    string s;
    while (x) {
        s.push_back('0' + (x & 1));
        x >>= 1;
    }
    reverse(s.begin(), s.end());
    cout << s << endl;
}
```

## o1预处理gcd

> 用途：利用前后缀 gcd 做删点/区间外 gcd 查询
> 复杂度：预处理 O(n)，单次查询 O(1)

```cpp
namespace fast_gcd {
    const int maxv = 1e6 + 5, B = 1e3 + 5;
    int pr[maxv], prn, a[maxv][3], g[B + 1][B + 1];
    bitset<maxv> vis;
    int gcd(int x, int y) {
        if (x > y) swap(x, y);
        if (!x) return y;
        if (x <= B) return g[x][y % x];
        int ans = 1, t;
        if (a[x][0] > 1) t = g[a[x][0]][y % a[x][0]], y /= t, ans *= t;
        if (a[x][1] > 1) t = g[a[x][1]][y % a[x][1]], y /= t, ans *= t;
        t = !vis[a[x][2]] ? (y % a[x][2] ? 1 : a[x][2]) : g[a[x][2]][y % a[x][2]];
        return ans * t;
    }
    void init() {
        a[1][0] = a[1][1] = a[1][2] = vis[0] = vis[1] = 1;
        for (int i = 2;i < maxv;i++) {
            if (!vis[i]) pr[++prn] = a[i][2] = i, a[i][0] = a[i][1] = 1;
            for (int j = 1;j <= prn && i * pr[j] < maxv;j++) {
                int v = i * pr[j];
                vis[v] = 1;
                a[v][0] = a[i][0] * pr[j], a[v][1] = a[i][1], a[v][2] = a[i][2];
                if (a[v][0] > a[v][1]) swap(a[v][0], a[v][1]);
                if (a[v][1] > a[v][2]) swap(a[v][1], a[v][2]);
                if (i % pr[j] == 0) break;
            }
        }
        for (int i = 1;i <= B;i++) {
            g[0][i] = g[i][0] = i;
            for (int j = 1;j <= i;j++)
                g[i][j] = g[j][i] = g[i % j][j];
        }
    }
}

using fast_gcd::gcd;
```

## Pollard Rho

> 用途：大整数质因数分解
> 复杂度：期望亚指数，工程上常用于 64 位范围快速分解

```cpp
namespace PR {
    ll p;
    ld invp;
    void setmod(ll x) {
        p = x, invp = (ld)1 / x;
    }
    ll mul(ll a, ll b) {
        ll z = a * invp * b + 0.5;
        ll res = a * b - z * p;
        return res + (res >> 63 & p);
    }
    ll ksm(ll a, ll x, ll res = 1) {
        for (;x;x >>= 1, a = mul(a, a))
            if (x & 1) res = mul(res, a);
        return res;
    }
    bool isprime(ll p) {
        if (p == 1) return 0;
        setmod(p);
        ll d = __builtin_ctzll(p - 1), s = (p - 1) >> d;
        for (ll a : { 2, 3, 5, 7, 11, 13, 82, 373 }) {
            if (a % p == 0)
                continue;
            ll x = ksm(a, s), y;
            for (int i = 0;i < d;++i, x = y) {
                y = mul(x, x);
                if (y == 1 && x != 1 && x != p - 1)
                    return 0;
            }
            if (x != 1) return 0;
        }
        return 1;
    }
    ll rho(ll n) {
        if (!(n & 1)) return 2;
        ll x = 0, y = 0, prod = 1;
        auto f = [&](ll o) { return mul(o, o) + 3; };//TLE的时候把这个3改成别的
        setmod(n);
        for (int t = 30, z = 0;t % 64 || gcd(prod, n) == 1;++t) {
            if (x == y) x = ++z, y = f(x);
            if (ll q = mul(prod, x + n - y)) prod = q;
            x = f(x), y = f(f(y));
        }
        return gcd(prod, n);
    }
    vector<ll> factor(ll x) {
        vector<ll> res;
        auto f = [&](auto f, ll x) {
            if (x == 1) return;
            if (isprime(x)) return res.push_back(x);
            ll y = rho(x);
            f(f, y), f(f, x / y);
            };
        f(f, x), sort(res.begin(), res.end());
        return res;
    }
}


void solve() {
    ll n;
    cin >> n;
    auto a = PR::factor(n);
}
```

## 多项式

> 用途：多项式乘法/求逆/ln/exp 等（常依赖 NTT）
> 复杂度：常见核心操作 O(n log n)

```cpp
const double PI = acos(-1);
struct Polynomial {
    struct Complex {
        double x, y;
        Complex(double _x = 0.0, double _y = 0.0) {
            x = _x;
            y = _y;
        }
        Complex operator-(const Complex& rhs) const {
            return Complex(x - rhs.x, y - rhs.y);
        }
        Complex operator+(const Complex& rhs) const {
            return Complex(x + rhs.x, y + rhs.y);
        }
        Complex operator*(const Complex& rhs) const {
            return Complex(x * rhs.x - y * rhs.y, x * rhs.y + y * rhs.x);
        }
    };
    vector<Complex> c;
    Polynomial(vector<int>& a) {
        int n = a.size();
        c.resize(n);
        for (int i = 0; i < n; i++) {
            c[i] = Complex(a[i], 0);
        }
        fft(c, n, 1);
    }
    void change(vector<Complex>& a, int n) {
        for (int i = 1, j = n / 2; i < n - 1; i++) {
            if (i < j) swap(a[i], a[j]);
            int k = n / 2;
            while (j >= k) {
                j -= k;
                k /= 2;
            }
            if (j < k) j += k;
        }
    }
    void fft(vector<Complex>& a, int n, int opt) {
        change(a, n);
        for (int h = 2; h <= n; h *= 2) {
            Complex wn(cos(2 * PI / h), sin(opt * 2 * PI / h));
            for (int j = 0; j < n; j += h) {
                Complex w(1, 0);
                for (int k = j; k < j + h / 2; k++) {
                    Complex u = a[k];
                    Complex t = w * a[k + h / 2];
                    a[k] = u + t;
                    a[k + h / 2] = u - t;
                    w = w * wn;
                }
            }
        }
        if (opt == -1) {
            for (int i = 0; i < n; i++) {
                a[i].x /= n;
            }
        }
    }
};

const int G = 3, MOD = 998244353; // for MOD = 998244353 = 2^23*7*17+1

ll ksm(ll a, ll b = MOD - 2) {
    ll res = 1;
    for (; b; b >>= 1) {
        if (b & 1) res = res * a % MOD;
        a = a * a % MOD;
    }
    return res;
}

int mod(int x) {
    return x >= MOD ? x - MOD : x;
}

void ntt(vector<int>& a, int flag) {
    int n = a.size();
    vector<int> gn(n);
    gn[0] = 1; gn[1] = ksm(G, flag == 1 ? (MOD - 1) / n : MOD - 1 - (MOD - 1) / n);
    for (int i = 2; i < n; i++) gn[i] = 1LL * gn[i - 1] * gn[1] % MOD;
    for (int i = 1, j = n / 2, k; i < n - 1; i++) {
        if (i < j) swap(a[i], a[j]);
        for (k = n / 2; j >= k; k >>= 1) j -= k;
        j += k;
    }
    for (int k = 2, step = n / 2; k <= n; k <<= 1, step >>= 1) {
        for (int i = 0; i < n; i += k) {
            for (int j = i, wi = 0; j < i + k / 2; j++, wi += step) {
                int u = a[j];
                int v = 1LL * gn[wi] * a[j + k / 2] % MOD;
                a[j] = mod(u + v);
                a[j + k / 2] = mod(u + MOD - v);
            }
        }
    }
    if (flag < 0) {
        ll inv_n = ksm(n);
        for (int i = 0; i < n; i++) a[i] = a[i] * inv_n % MOD;
    }
}

vector<int> mul(vector<int> f, vector<int> g) {
    int n = f.size(), m = g.size();
    int len = 1LL << (__lg(n + m - 2) + 1);
    f.resize(len); g.resize(len);
    ntt(f, 1); ntt(g, 1);
    for (int i = 0; i < len; i++) f[i] = 1LL * f[i] * g[i] % MOD;
    ntt(f, -1);
    f.resize(n + m - 1);
    return f;
}
```

## 根号算n以内质数个数

> 用途：快速计算 π(n)（n 以内质数个数）
> 复杂度：常见做法约 O(n^(2/3))（依实现）

```cpp
ll f[340000], g[340000];
ll lehmer_pi(ll n) {
    ll i, j, m;
    for (m = 1;m * m <= n;++m)f[m] = n / m - 1;
    for (i = 1;i <= m;++i)g[i] = i - 1;
    for (i = 2;i <= m;++i) {
        if (g[i] == g[i - 1])continue;
        for (j = 1;j <= min(m - 1, n / i / i);++j) {
            if (i * j < m)f[j] -= f[i * j] - g[i - 1];
            else f[j] -= g[n / i / j] - g[i - 1];
        }
        for (j = m;j >= i * i;--j)g[j] -= g[j / i] - g[i - 1];
    }
    return f[1];
}
```

## 勾股数

> 用途：勾股数构造、判定与相关筛法
> 复杂度：视实现而定（常见 O(n)~O(n log n)）

```cpp
 
vector<PII>st[300005];

for (int i = 1;i * i <= 3e5;i++) {
    for (int j = 1;j < i && i * i + j * j <= 3e5;j++) {
        if ((i + j + 1) & 1)continue;
        if (gcd(i, j) != 1)continue;
        int a = i * i - j * j;
        int b = 2 * i * j;
        int c = i * i + j * j;
        if (a > b)swap(a, b);
        for (int x = 1;x * c <= 3e5;x++) {
            st[x * c].push_back({ x * a,x * b });
        }
    }
}

圆上整点:
ll r, ans;
void cal(int d) {
    for (ll s = 1;s * s < r / d;s++) {
        ll t = sqrtl(2 * r / d - s * s);
        if (t * t == 2 * r / d - s * s && gcd(s, t) == 1) {
            // ll x = d * s * t, y = d * (t * t - s * s) / 2;
            ans += 4;
        }
    }
}

void solve() {
    cin >> r;
    for (ll i = 1;i * i <= 2 * r;i++) {
        if ((2 * r) % i == 0) {
            cal(i);
            if (i * i != 2 * r)cal(2 * r / i);
        }
    }
    cout << ans + 4;
}
```

## 矩阵快速幂

> 用途：线性递推、图路径计数等
> 复杂度：O(k^3 log n)（k 为矩阵维度）

```cpp
int len;
struct mx {ll a[5][5];};
mx mul(mx &a, mx &b) {
    mx c;
    for (int i = 1; i <= len; i++) {
        for (int j = 1; j <= len; j++) {
            c.a[i][j] = 0;
            for (int k = 1; k <= len; k++) {
                c.a[i][j] = (c.a[i][j] + a.a[i][k] * b.a[k][j] % M + M) % M;
            }
        }
    }
    return c;
}
mx ksm(mx a, ll k) {
    mx res;
    for (int i = 1; i <= len; i++)for (int j = 1; j <= len; j++) {
        res.a[i][j] = (i == j);
    }
    while (k) {
        if (k & 1)res = mul(res, a);
        a = mul(a, a);
        k >>= 1;
    }
    return res;
}

```

## 莫比乌斯函数

> 用途：反演、互质计数、积性函数前缀问题
> 复杂度：线性筛 O(n)

```cpp
bool st[N];
int mu[N], prime[N], cnt, sum[N];
void getMu(int n) {
    mu[1] = 1;
    for (int i = 2; i <= n; i++) {
        if (!st[i]) {
            prime[++cnt] = i;
            mu[i] = -1;
        }
        for (int j = 1; j <= cnt && i * prime[j] <= n; j++) {
            st[i * prime[j]] = true;
            if (i % prime[j] == 0) {
                mu[i * prime[j]] = 0;
                break;
            }
            mu[i * prime[j]] = -mu[i];
        }
    }
    for (int i = 1; i <= n; i++) {
        sum[i] = sum[i - 1] + mu[i];
    }
}
```

## 欧拉筛

> 用途：线性筛质数与积性函数
> 复杂度：O(n)

```cpp
vector<int> minp, primes;
void sieve(int n) {
    minp.assign(n + 1, 0);
    primes.clear();
    for (int i = 2; i <= n; i++) {
        if (minp[i] == 0) {
            minp[i] = i;
            primes.push_back(i);
        }
        for (auto p : primes) {
            if (i * p > n) {
                break;
            }
            minp[i * p] = p;
            if (p == minp[i]) {
                break;
            }
        }
    }
}

bool notp[N];
int p[N], cnt;
void sieve(int n) {
    for (int i = 2;i <= n;i++) {
        if (!notp[i])p[++cnt] = i;
        for (int j = 1;j <= cnt && i * p[j] <= n;j++) {
            notp[i * p[j]] = 1;
            if (i % p[j] == 0)break;
        }
    }
}

```

## 线性基

> 用途：异或空间基、最大异或/可表示性判断
> 复杂度：插入 O(bit)，查询 O(bit)

```cpp
高斯消元:
int n, k;
ll a[N];
void gauss() {
    k = 1;
    for (int i = 62;i >= 0;i--) {
        for (int j = k;j <= n;j++) {
            if (a[j] >> i & 1) {
                swap(a[k], a[j]);
                break;
            }
        }
        if (!(a[k] >> i & 1))continue;
        for (int j = 1;j <= n;j++) {
            if (j != k && (a[j] >> i & 1)) {
                a[j] ^= a[k];
            }
        }
        k++;
        if (k == n + 1)break;
    }
    k--;
}

贪:
ll p[64];
void insert(ll x) {
    for (int i = 63;i >= 0;i--) {
        if (x >> i & 1) {
            if (!p[i]) {
                p[i] = x;
                break;
            }
            x ^= p[i];
        }
    }
}

struct base {
    vector<ll> b;
    int m;
    base() {}
    base(int m_) {
        init(m_);
    }
    void init(int m_) {
        m = m_;
        b.assign(m + 1, 0);
    }
    void insert(ll x) {
        for (int i = m;i >= 0;i--) {
            if (x >> i & 1) {
                if (!b[i]) {
                    b[i] = x;
                    break;
                }
                x ^= b[i];
            }
        }
    }
    ll qry(ll x) {
        for (int i = m;i >= 0;i--) {
            x = max(x, x ^ b[i]);
        }
        return x;
    }
    bool exist(ll x) {
        for (int i = m;i >= 0;i--) {
            if (x & (1LL << i)) {
                if (!b[i])return 0;
                x ^= b[i];
            }
        }
        return 1;
    }
};


struct LB { // Linear Basis
    using i64 = long long;
    const int BASE = 63;
    vector<i64> d, p;
    int cnt, flag;
    LB() {
        d.resize(BASE + 1);
        p.resize(BASE + 1);
        cnt = flag = 0;
    }
    bool insert(i64 val) {
        for (int i = BASE - 1; i >= 0; i--) {
            if (val & (1ll << i)) {
                if (!d[i]) {
                    d[i] = val;
                    return true;
                }
                val ^= d[i];
            }
        }
        flag = 1; //可以异或出0
        return false;
    }
    bool check(i64 val) { // 判断 val 是否能被异或得到
        for (int i = BASE - 1; i >= 0; i--) {
            if (val & (1ll << i)) {
                if (!d[i]) {
                    return false;
                }
                val ^= d[i];
            }
        }
        return true;
    }
    i64 ask_max() {
        i64 res = 0;
        for (int i = BASE - 1; i >= 0; i--) {
            if ((res ^ d[i]) > res) res ^= d[i];
        }
        return res;
    }
    i64 ask_min() {
        if (flag) return 0; // 特判 0
        for (int i = 0; i <= BASE - 1; i++) {
            if (d[i]) return d[i];
        }
    }
    void rebuild() { // 第k小值独立预处理
        for (int i = BASE - 1; i >= 0; i--) {
            for (int j = i - 1; j >= 0; j--) {
                if (d[i] & (1ll << j)) d[i] ^= d[j];
            }
        }
        for (int i = 0; i <= BASE - 1; i++) {
            if (d[i]) p[cnt++] = d[i];
        }
    }
    i64 kthquery(i64 k) { // 查询能被异或得到的第 k 小值, 如不存在则返回 -1
        if (flag) k--; // 特判 0, 如果不需要 0, 直接删去
        if (!k) return 0;
        i64 res = 0;
        if (k >= (1ll << cnt)) return -1;
        for (int i = BASE - 1; i >= 0; i--) {
            if (k & (1LL << i)) res ^= p[i];
        }
        return res;
    }
    void Merge(const LB& b) { // 合并两个线性基
        for (int i = BASE - 1; i >= 0; i--) {
            if (b.d[i]) {
                insert(b.d[i]);
            }
        }
    }
};
```

## 行列式

> 用途：矩阵是否可逆、体积缩放、线性方程组相关
> 复杂度：高斯消元 O(n^3)

```cpp
template <class T>
struct Point {
  T x, y;

  Point(T x = 0, T y = 0) : x(x), y(y) {}

  friend Point operator+(const Point &a, const Point &b) {
    return {a.x + b.x, a.y + b.y};
  }

  friend Point operator-(const Point &a, const Point &b) {
    return {a.x - b.x, a.y - b.y};
  }

  // 点乘
  friend T operator*(const Point &a, const Point &b) {
    return a.x * b.x + a.y * b.y;
  }

  // 叉乘
  friend T operator^(const Point &a, const Point &b) {
    return a.x * b.y - a.y * b.x;
  }
};

template <class T>
vector<Point<T>> minkowski_sum(vector<Point<T>> a, vector<Point<T>> b) {
  vector<Point<T>> c{a[0] + b[0]};
  for (usz i = 0; i + 1 < a.size(); ++i) a[i] = a[i + 1] - a[i];
  for (usz i = 0; i + 1 < b.size(); ++i) b[i] = b[i + 1] - b[i];
  a.pop_back(), b.pop_back();
  c.resize(a.size() + b.size() + 1);
  merge(a.begin(), a.end(), b.begin(), b.end(), c.begin() + 1,
        [](const Point<i64> &a, const Point<i64> &b) { return (a ^ b) < 0; });
  for (usz i = 1; i < c.size(); ++i) c[i] = c[i] + c[i - 1];
  return c;
}
```

## 组合数

> 用途：组合数 C(n,k) 计算（模素数/非素数扩展）
> 复杂度：预处理阶乘逆元 O(n)，单次 O(1)

```cpp
int c[505][505];
void init(int n) {
    for (int i = 0; i <= n; i++) {
        for (int j = 0; j <= i; j++){
            if (!j) c[j][i] = 1;
            else c[j][i] = (c[j][i - 1] + c[j - 1][i - 1]) % M;
        }
    }
}

lucas:
C(b, a) % p = C(b / p, a / p) * C(b % p, a % p)%p
ll lucas(ll n, ll m, ll p) {
    if (!m)return 1;
    return C(n % p, m % p, p) * lucas(n / p, m / p, p) % p;
}

void init(ll p) {
    fac[0] = inv[0] = fac[1] = inv[1] = 1;
    for (int i = 2; i < p; i++) {
        fac[i] = fac[i - 1] * i % p;
        inv[i] = (p - p / i) * inv[p % i] % p;
    }
    for (int i = 2;i < p;i++) {
        inv[i] = inv[i] * inv[i - 1] % p;
    }
}

ll fac[N + 5], inv[N + 5];
ll C(ll n, ll m) {
	if (m > n)return 0;
	return fac[n] * inv[n - m] % M * inv[m] % M;
}
ll ksm(ll a, ll b = M - 2) {
	ll s = 1;
    for (; b > 0; b >>= 1, (a *= a) %= M)if (b & 1)(s *= a) %= M;
	return s;
}
void init() {
    fac[0] = inv[0] = 1;
	for (int i = 1; i <= N; i++)fac[i] = fac[i - 1] * i % M;
	inv[N - 1] = ksm(fac[N - 1]);
    for (int i = N - 2; i > 0; i--)inv[i] = inv[i + 1] * (i + 1) % M;
}

第二类斯特林数:
ll stl[N][N];
void init(int n) {
    stl[0][0] = 1;
    for (int i = 1;i <= n;i++) {
        for (int j = 1;j <= i;j++) {
            stl[i][j] = (stl[i - 1][j - 1] + j * stl[i - 1][j]) % M;
        }
    }
}
```

